...
class DuelGroup{
	...
	public function initializeWin($player) : void{
		if(Utils::isPlayer($player)){
			if(!is_null($player)){
				...
				if($ranked === true){
					if($player instanceof CustomPlayer) Utils::matchOutcome($player, 0, true);
				}else{
					if($player instanceof CustomPlayer) Utils::matchOutcome($player, 0, false);
				}
			}
		}
	}
	...
	public function initializeLoss($player) : void{
		if(Utils::isPlayer($player)){
			if(!is_null($player)){
				...
				if($ranked === true){
					if($player instanceof CustomPlayer) Utils::matchOutcome($player, 1, true);
				}else{
					if($player instanceof CustomPlayer) Utils::matchOutcome($player, 1, false);
				}
			}
		}
	}
	...
	private function endDuel(bool $endPrematurely = false, bool $disablePlugin = false, $win = false) : void{
		...
		if($player instanceof CustomPlayer) $player->setTagged(false);
		if($opponent instanceof CustomPlayer) $opponent->sendTo(0, true);
		if($opponent instanceof CustomPlayer) $opponent->setTagged(false);
		if($win === true){
			if($winner !== self::NONE and $loser !== self::NONE){
				if($ranked === true){
