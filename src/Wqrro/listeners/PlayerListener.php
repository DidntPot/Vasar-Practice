...
use Wqrro\Kits;
use Wqrro\LevelUtils;
use Wqrro\tasks\onetime\ChatCooldownTask;
use Wqrro\tasks\onetime\CloseEntityTask;
use Wqrro\tasks\onetime\PearlTask;
use Wqrro\Utils;
...
class PlayerListener implements Listener
{
    ...
    {
        $player = $event->getPlayer();
        if ($player instanceof CustomPlayer) $player->initializeJoin();
    ...
    public function onQuit(PlayerQuitEvent $event)
    {
        ...
        if ($player instanceof CustomPlayer) $player->initializeQuit();
        $event->setQuitMessage("");
        if ($player instanceof CustomPlayer and $player->isTagged()) {
    ...
    public function onDeath(PlayerDeathEvent $event)
    {
        ...
        if ($cause instanceof EntityDamageByEntityEvent and $cause->getDamager() !== null) {
            ...
            if ($player instanceof Player and $killer instanceof Player) {
                ...
                if ($killer instanceof CustomPlayer) LevelUtils::increaseCurrentXp($killer, "kill", false);
                if ($killer instanceof CustomPlayer) LevelUtils::checkXp($killer);
                if ($player instanceof CustomPlayer) Utils::updateStats($player, 1);
                if ($player instanceof CustomPlayer) LevelUtils::checkXp($player);
    ...
    public function onRespawn(PlayerRespawnEvent $event)
    {
        ...
        if ($player instanceof CustomPlayer) $player->sendTo(0, true);
    }
    ...
    public function onCommand(PlayerCommandPreprocessEvent $event)
    {
        $player = $event->getPlayer();
        $message = $event->getMessage();
        if ($player instanceof CustomPlayer and $player->isFrozen() and $message[0] === "/") {
            $event->setCancelled();
        }
    }
            ...
            if ($this->plugin->getDuelHandler()->isInBotDuel($player)) {
                ...
                if ($event->getFinalDamage() - $check >= $player->getHealth()) {
                    if (!$player->isCreative()) {
                        ...
                        if ($player instanceof CustomPlayer) Utils::spawnLightning($player);
                    }
                }
                return;
            }
            ...
            if ($player instanceof Player and $damager instanceof Player) {
                ...
                foreach ([$player, $damager] as $players) {
                    if (!$event->isCancelled()) {
                        if ($players instanceof CustomPlayer) $players->setTagged(true);
                    }
                ...
                if ($player instanceof CustomPlayer and $player->isFrozen()) {
                    $event->setCancelled();
                    $damager->sendMessage("§cYou cannot damage a frozen player.");
                }
                if ($player instanceof CustomPlayer and $player->isVanished()) {
                    $event->setCancelled();
                }
                if ($damager->isFrozen()) {
                    $event->setCancelled();
